...
Codex 默认使用当前会话串行或分批拆解。多 agent 只作为用户明确要求时的可选增强；没有 `.claude` 目录时，深度拆解仍按下方分块策略推进。
...
| 2 | 逐章摘要 | 分块章节文本 | 章节摘要.md（含情节点+角色）。角色过滤（龙套不提取、别名归类）。每章3-40情节点（密度150-200字/个，按字数动态调节）。Codex 默认串行/批处理；legacy 可选并行模式每章 spawn chapter-extractor agent。**计数验证：摘要数 == 章节数，不等则标记失败章节**。 | 所有章节处理完成 |
...
## Legacy/Claude Compatibility：Stage 2 并行 Agent 策略

Codex 默认 Stage 2 由当前会话按章节串行或分批处理。只有用户明确要求多 agent/Claude 兼容模式时，才使用 chapter-extractor agent 并行处理每章，替代串行分块。
...
Stage 2 在 Codex 默认路径中按章节串行/批处理。Legacy/Claude Compatibility 模式可使用 chapter-extractor agent 按章节并行，不分块。
